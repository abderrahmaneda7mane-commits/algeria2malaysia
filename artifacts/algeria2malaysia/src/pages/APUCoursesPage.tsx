...
function SkeletonCard() {
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-5 animate-pulse">
      ...
      <div className="flex gap-3"><div className="h-4 bg-gray-100 rounded w-1/4"></div><div className="h-4 bg-gray-100 rounded w-1/4"></div><div className="h-4 bg-gray-100 rounded w-1/4"></div></div>
    </div>
  );
}
...
export default function APUCoursesPage() {
  ...
  } = useCourses(APU_UNIVERSITY_ID, PRICE_RANGES);
    ...
    <div className="min-h-screen bg-gray-50" dir="rtl">
      <div className="bg-gradient-to-br from-blue-900 via-blue-800 to-blue-700 text-white pt-12 pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <button onClick={() => go("universities")} className="flex items-center gap-2 text-blue-200 hover:text-white transition-colors mb-6 group">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <div className="inline-block bg-blue-500/40 border border-blue-400/40 rounded-full px-3 py-0.5 text-xs font-semibold text-blue-100 mb-1">جامعة تقنية · Top Tech</div>
              <h1 className="text-2xl md:text-3xl font-bold">جامعة APU (Asia Pacific University)</h1>
              <p className="text-blue-200 text-sm mt-0.5">Asia Pacific University of Technology & Innovation — KL</p>
            </div>
          </div>
          {!loading && !error && (
            <div className="mt-4 inline-flex items-center gap-2 bg-white/15 border border-white/20 rounded-full px-4 py-1.5 text-sm font-medium">
              <GraduationCap size={15} /><span>{total.toLocaleString()} مادة دراسية</span>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="relative">
              ...
              <input type="text" placeholder="ابحث باسم المادة..." value={search} onChange={e => setSearch(e.target.value)}
                className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none bg-white transition" />
            </div>
            <div className="relative">
            ...
            <div className="relative">
              ...
              <select dir="ltr" value={priceRangeIdx} onChange={e => setPriceRangeIdx(Number(e.target.value))}
                className="w-full border border-gray-200 rounded-xl py-2.5 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none bg-white transition text-left">
        ...
        {!loading && !error && courses.length > 0 && (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              {courses.map(course => (
                <div key={course.id} className="bg-white border border-gray-100 rounded-2xl p-5 hover:shadow-md hover:border-blue-200 transition-all">
                  <h3 className="font-bold text-gray-900 text-base leading-snug mb-1">{course.name}</h3>
                  <p className="text-blue-700 text-sm font-medium mb-4">{course.universities?.[0]?.name ?? "APU"}</p>
                  <div className="flex flex-wrap gap-x-5 gap-y-2 text-sm text-gray-600">
                    {course.duration != null && <span className="flex items-center gap-1.5"><Clock size={14} className="text-blue-500 flex-shrink-0" />{course.duration} {Number(course.duration) === 1 ? "سنة" : "سنوات"}</span>}
                    {course.intake && <span className="flex items-center gap-1.5"><Calendar size={14} className="text-blue-500 flex-shrink-0" />{course.intake.trim()}</span>}
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 flex-wrap">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="flex items-center gap-1 px-4 py-2 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 hover:bg-blue-50 hover:border-blue-300 disabled:opacity-40 disabled:cursor-not-allowed transition-all"><ChevronRight size={16} />السابق</button>
                {pageNumbers.map((num, i) => num === "..." ? <span key={`e-${i}`} className="px-2 text-gray-400">…</span> :
                  <button key={num} onClick={() => setPage(num)} className={`w-10 h-10 rounded-xl text-sm font-semibold transition-all border ${page === num ? "bg-blue-600 text-white border-blue-600 shadow-md" : "bg-white text-gray-700 border-gray-200 hover:bg-blue-50 hover:border-blue-300"}`}>{num}</button>)}
                <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="flex items-center gap-1 px-4 py-2 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 hover:bg-blue-50 hover:border-blue-300 disabled:opacity-40 disabled:cursor-not-allowed transition-all">التالي<ChevronLeft size={16} /></button>
              </div>
            )}
