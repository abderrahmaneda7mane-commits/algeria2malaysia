...
export default function UPMCoursesPage() {
    ...
    async function fetchCourses() {
      ...
      try {
        const { data, error: sbError } = await supabase
          ...
          .eq("university_id", UPM_UNIVERSITY_ID);
  ...
  const universityName = courses[0]?.universities?.[0]?.name ?? "Universiti Putra Malaysia (UPM)";
      ...
      <div className="bg-gradient-to-br from-emerald-900 via-emerald-800 to-emerald-700 text-white pt-12 pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <button
            ...
            className="flex items-center gap-2 text-emerald-200 hover:text-white transition-colors mb-6 group"
          >
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <div className="inline-block bg-emerald-500/40 border border-emerald-400/40 rounded-full px-3 py-0.5 text-xs font-semibold text-emerald-100 mb-1">
                جامعة حكومية · Research
              </div>
              <h1 className="text-2xl md:text-3xl font-bold">جامعة بوترا ماليزيا (UPM)</h1>
              <p className="text-emerald-200 text-sm mt-0.5">{universityName}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition"
              />
            </div>
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent appearance-none bg-white transition"
              >
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full border border-gray-200 rounded-xl py-2.5 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent appearance-none bg-white transition text-left"
              >
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              {paginated.map((course) => (
                <div
                  key={course.id}
                  className="bg-white border border-gray-100 rounded-2xl p-5 hover:shadow-md hover:border-emerald-200 transition-all"
                >
                  <h3 className="font-bold text-gray-900 text-base leading-snug mb-1">
                    {course.name}
                  </h3>
                  <p className="text-emerald-700 text-sm font-medium mb-4">
                    {course.universities?.[0]?.name ?? "UPM"}
                  </p>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 flex-wrap">
                <button
                  ...
                  disabled={page === 1}
                  className="flex items-center gap-1 px-4 py-2 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 hover:bg-emerald-50 hover:border-emerald-300 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                >
