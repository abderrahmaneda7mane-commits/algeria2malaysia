...
const PRICE_RANGES = [
  ...
  { label: "RM 20,000 – 35,000 · €4,000 – €7,000", min: 20000, max: 35000 },
  { label: "RM +35,000 · €7,000+", min: 35000, max: Infinity },
];
...
export default function UTMCoursesPage() {
  ...
  useEffect(() => {
    async function fetchCourses() {
      ...
      try {
        const { data, error: sbError } = await supabase.from("courses").select("id, name, duration, intake, price, universities(name)").eq("university_id", UTM_UNIVERSITY_ID);
      ...
      <div className="bg-gradient-to-br from-sky-900 via-sky-800 to-sky-700 text-white pt-12 pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <div className="inline-block bg-sky-500/40 border border-sky-400/40 rounded-full px-3 py-0.5 text-xs font-semibold text-sky-100 mb-1">جامعة حكومية · Research</div>
              <h1 className="text-2xl md:text-3xl font-bold">جامعة تكنولوجيا ماليزيا (UTM)</h1>
              <p className="text-sky-200 text-sm mt-0.5">Universiti Teknologi Malaysia — Johor Bahru & KL</p>
            </div>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 -mt-6 mb-6 relative z-10">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="relative"><Search size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" /><input type="text" placeholder="ابحث باسم المادة..." value={search} onChange={e => handleFilterChange(() => setSearch(e.target.value))} className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent transition" /></div>
            <div className="relative"><Clock size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" /><select value={durationFilter} onChange={e => handleFilterChange(() => setDurationFilter(e.target.value))} className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 appearance-none bg-white transition"><option value="all">كل المدد الدراسية</option>{durations.filter(d => d !== "all").map(d => <option key={d} value={d}>{d} {Number(d) === 1 ? "سنة" : "سنوات"}</option>)}</select></div>
            <div className="relative"><DollarSign size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" /><select dir="ltr" value={priceRangeIdx} onChange={e => handleFilterChange(() => setPriceRangeIdx(Number(e.target.value)))} className="w-full border border-gray-200 rounded-xl py-2.5 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 appearance-none bg-white transition text-left">{PRICE_RANGES.map((r, i) => <option key={i} value={i}>{r.label}</option>)}</select></div>
          </div>
        </div>
      </div>
