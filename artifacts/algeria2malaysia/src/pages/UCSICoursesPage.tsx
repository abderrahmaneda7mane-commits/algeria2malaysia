...
const PRICE_RANGES = [
  ...
  { label: "RM 5,000 – 20,000 · €1,000 – €4,000", min: 5000, max: 20000 },
  { label: "RM 20,000 – 40,000 · €4,000 – €8,000", min: 20000, max: 40000 },
...
export default function UCSICoursesPage() {
  ...
  useEffect(() => {
    async function fetchCourses() {
      ...
      try {
        const { data, error: sbError } = await supabase.from("courses").select("id, name, duration, intake, price, universities(name)").eq("university_id", UCSI_UNIVERSITY_ID);
      ...
      <div className="bg-gradient-to-br from-pink-900 via-pink-800 to-pink-700 text-white pt-12 pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <div className="inline-block bg-pink-500/40 border border-pink-400/40 rounded-full px-3 py-0.5 text-xs font-semibold text-pink-100 mb-1">جامعة خاصة · Medicine & Business</div>
              <h1 className="text-2xl md:text-3xl font-bold">جامعة UCSI</h1>
              <p className="text-pink-200 text-sm mt-0.5">UCSI University Malaysia — Kuala Lumpur</p>
            </div>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 -mt-6 mb-6 relative z-10">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="relative"><Search size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" /><input type="text" placeholder="ابحث باسم المادة..." value={search} onChange={e => handleFilterChange(() => setSearch(e.target.value))} className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-pink-500 focus:border-transparent transition" /></div>
            <div className="relative"><Clock size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" /><select value={durationFilter} onChange={e => handleFilterChange(() => setDurationFilter(e.target.value))} className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-pink-500 appearance-none bg-white transition"><option value="all">كل المدد الدراسية</option>{durations.filter(d => d !== "all").map(d => <option key={d} value={d}>{d} {Number(d) === 1 ? "سنة" : "سنوات"}</option>)}</select></div>
            <div className="relative"><DollarSign size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" /><select dir="ltr" value={priceRangeIdx} onChange={e => handleFilterChange(() => setPriceRangeIdx(Number(e.target.value)))} className="w-full border border-gray-200 rounded-xl py-2.5 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-pink-500 appearance-none bg-white transition text-left">{PRICE_RANGES.map((r, i) => <option key={i} value={i}>{r.label}</option>)}</select></div>
          </div>
        </div>
      </div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              {paginated.map(course => (
                <div key={course.id} className="bg-white border border-gray-100 rounded-2xl p-5 hover:shadow-md hover:border-pink-200 transition-all">
                  <h3 className="font-bold text-gray-900 text-base leading-snug mb-1">{course.name}</h3>
                  <p className="text-pink-700 text-sm font-medium mb-4">{course.universities?.[0]?.name ?? "UCSI University"}</p>
