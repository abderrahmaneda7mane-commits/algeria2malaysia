...
export default function UTPCoursesPage() {
  ...
  useEffect(() => {
    async function fetchCourses() {
      ...
      try {
        const { data, error: sbError } = await supabase.from("courses").select("id, name, duration, intake, price, universities(name)").eq("university_id", UTP_UNIVERSITY_ID);
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir="rtl">
      <div className="bg-gradient-to-br from-yellow-900 via-yellow-800 to-yellow-700 text-white pt-12 pb-16 px-4">
        <div className="max-w-5xl mx-auto">
          <button onClick={() => go("universities")} className="flex items-center gap-2 text-yellow-200 hover:text-white transition-colors mb-6 group"><ArrowLeft size={18} className="group-hover:-translate-x-1 transition-transform" /><span>العودة إلى الجامعات</span></button>
          <div className="flex items-center gap-4 mb-4">
            <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center flex-shrink-0 p-2 shadow-lg"><img src="/logos/utp.png" alt="UTP" className="max-w-full max-h-full object-contain" /></div>
            <div>
              <div className="inline-block bg-yellow-500/40 border border-yellow-400/40 rounded-full px-3 py-0.5 text-xs font-semibold text-yellow-100 mb-1">جامعة هندسية · Top Engineering</div>
              <h1 className="text-2xl md:text-3xl font-bold">جامعة تكنولوجيا بتروناس (UTP)</h1>
              <p className="text-yellow-200 text-sm mt-0.5">Universiti Teknologi PETRONAS — Seri Iskandar</p>
            </div>
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 -mt-6 mb-6 relative z-10">
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="relative"><Search size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" /><input type="text" placeholder="ابحث باسم المادة..." value={search} onChange={e => handleFilterChange(() => setSearch(e.target.value))} className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent transition" /></div>
            <div className="relative"><Clock size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" /><select value={durationFilter} onChange={e => handleFilterChange(() => setDurationFilter(e.target.value))} className="w-full border border-gray-200 rounded-xl py-2.5 pr-9 pl-3 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 appearance-none bg-white transition"><option value="all">كل المدد الدراسية</option>{durations.filter(d => d !== "all").map(d => <option key={d} value={d}>{d} {Number(d) === 1 ? "سنة" : "سنوات"}</option>)}</select></div>
            <div className="relative"><DollarSign size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" /><select dir="ltr" value={priceRangeIdx} onChange={e => handleFilterChange(() => setPriceRangeIdx(Number(e.target.value)))} className="w-full border border-gray-200 rounded-xl py-2.5 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500 appearance-none bg-white transition text-left">{PRICE_RANGES.map((r, i) => <option key={i} value={i}>{r.label}</option>)}</select></div>
          </div>
        </div>
      </div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              {paginated.map(course => (
                <div key={course.id} className="bg-white border border-gray-100 rounded-2xl p-5 hover:shadow-md hover:border-yellow-200 transition-all">
                  <h3 className="font-bold text-gray-900 text-base leading-snug mb-1">{course.name}</h3>
                  <p className="text-yellow-700 text-sm font-medium mb-4">{course.universities?.[0]?.name ?? "UTP"}</p>
                  <div className="flex flex-wrap gap-x-5 gap-y-2 text-sm text-gray-600">
                    {course.duration != null && <span className="flex items-center gap-1.5"><Clock size={14} className="text-yellow-600 flex-shrink-0" />{course.duration} {Number(course.duration) === 1 ? "سنة" : "سنوات"}</span>}
                    {course.intake && <span className="flex items-center gap-1.5"><Calendar size={14} className="text-yellow-600 flex-shrink-0" />{course.intake.trim()}</span>}
                    <span className="flex items-center gap-1.5"><DollarSign size={14} className="text-yellow-600 flex-shrink-0" />{course.price == null || course.price === 0 ? <span className="text-gray-400 italic">السعر غير متوفر</span> : <><span className="font-semibold text-green-700">{course.price.toLocaleString()} RM</span><span className="text-gray-400 text-xs">/ € {toEur(course.price)}</span><span className="text-gray-400 text-xs">· / سنة</span></>}</span>
                  </div>
                </div>
              ))}
            </div>
            {totalPages > 1 && (<div className="flex items-center justify-center gap-2 flex-wrap"><button onClick={() => { setPage(p => Math.max(1, p - 1)); window.scrollTo({ top: 0, behavior: "smooth" }); }} disabled={page === 1} className="flex items-center gap-1 px-4 py-2 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 hover:bg-yellow-50 hover:border-yellow-300 disabled:opacity-40 disabled:cursor-not-allowed transition-all"><ChevronRight size={16} />السابق</button>{pageNumbers.map((num, i) => num === "..." ? <span key={`e-${i}`} className="px-2 text-gray-400">…</span> : <button key={num} onClick={() => { setPage(num); window.scrollTo({ top: 0, behavior: "smooth" }); }} className={`w-10 h-10 rounded-xl text-sm font-semibold transition-all border ${page === num ? "bg-yellow-600 text-white border-yellow-600 shadow-md" : "bg-white text-gray-700 border-gray-200 hover:bg-yellow-50 hover:border-yellow-300"}`}>{num}</button>)}<button onClick={() => { setPage(p => Math.min(totalPages, p + 1)); window.scrollTo({ top: 0, behavior: "smooth" }); }} disabled={page === totalPages} className="flex items-center gap-1 px-4 py-2 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 hover:bg-yellow-50 hover:border-yellow-300 disabled:opacity-40 disabled:cursor-not-allowed transition-all">التالي<ChevronLeft size={16} /></button></div>)}
